    ...
    <html>
      ...
      <body>
        ...
        <div style="margin-top: 2em; padding: 1em; background: #f0f9ff; border-left: 4px solid #2563eb; border-radius: 4px;">
          <strong>Note:</strong> Full transcript and call recording are available in the AirDesk platform. Please log in to view the complete details.
    ...
    if (firm && firm.notify_emails && firm.notify_emails.length > 0) {
      try {
        await sendIntakeEmail(
          ...
          false, // sendIncompleteTickets
          undefined, // ccEmails
          call.id, // Call ID for dashboard link
          (firm as any).service_call_fee // Service call fee
        );
