...
import { buildVapiAssistantName, cleanVapiPayload } from '@/lib/vapi/utils';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const agentConfig = buildVapiAgent(
      firm.firm_name || 'the business',
      firm.agent_name || null,
      firm.ai_greeting_custom,
      firm.ai_knowledge_base || null, // Use knowledge base for business information
      firm.default_next_available_slot || null,
      firm.service_fee_enabled ?? true,
      firm.service_call_fee || null
    );

    if (!assistantId) {
      // Create new assistant
      const assistantPayload: any = {
        name: buildVapiAssistantName(firm.firm_name || firm.business_name, 'HVAC Receptionist'),
      ...
      try {
        ...
        const errorStatus = vapiError?.response?.status;
        const message =
          typeof errorDetails === 'string'
            ? errorDetails
            : (errorDetails?.message
                ? (Array.isArray(errorDetails.message) ? errorDetails.message.join(', ') : errorDetails.message)
                : (errorDetails?.error || vapiError?.message || 'Unknown Vapi error'));
