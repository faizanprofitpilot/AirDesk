    ...
    if (body.message) {
      ...
      if (message.type === 'status-update') {
        ...
        if (message.status === 'ended') {
          ...
          console.log('[Vapi Webhook] Call ended. Ended reason:', message.endedReason);
        } else {
      ...
      // Extract transcript from artifact.messages if available (for status-update)
      if (!transcript && message.artifact?.messages && Array.isArray(message.artifact.messages)) {
        console.log('[Vapi Webhook] Building transcript from artifact.messages array');
        const messages = message.artifact.messages
          .filter((msg: any) => {
            // Include user/assistant messages or transcript messages
            return (msg.role === 'user' || msg.role === 'assistant' || msg.role === 'bot') ||
                   msg.type === 'transcript' ||
                   (msg.content && msg.content.trim().length > 0);
          })
          .map((msg: any) => {
            const role = msg.role === 'user' ? 'Caller' : 
                        (msg.role === 'assistant' || msg.role === 'bot' ? 'AI Receptionist' : 'System');
            const content = msg.content || msg.message || msg.transcript || msg.text || '';
            return `${role}: ${content}`;
          });
      ...
      // Also try to extract from artifact.messagesOpenAIFormatted if available
      // This contains the conversation in OpenAI format which we can parse
      if (!structuredData && message.artifact?.messagesOpenAIFormatted && Array.isArray(message.artifact.messagesOpenAIFormatted)) {
        console.log('[Vapi Webhook] Attempting to extract structured data from messagesOpenAIFormatted');
        // The messages array contains the conversation - we can extract info from it
        // But structured data extraction from messages is complex, so we'll rely on Vapi's structuredData field
        // This is just a fallback - the main structured data should come from artifact.structuredData
      }
