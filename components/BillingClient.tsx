  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white rounded-xl shadow-sm" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }}>
        <CardHeader>
          <CardTitle style={{ color: '#1E40AF' }}>Current Subscription</CardTitle>
          ...
          {hasActiveSubscription ? (
            <>
              <div className="grid gap-6 md:grid-cols-2">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#475569' }}>
                    Current Plan
                  </div>
              ...
              {!firm.subscription_cancel_at_period_end && (
                <div className="border-t border-gray-200 pt-6">
                  <div className="text-xs font-semibold uppercase tracking-wide mb-4" style={{ color: '#475569' }}>
                    Change Plan
                  </div>
            ...
            <div className="text-center py-8">
              <p className="text-sm mb-4" style={{ color: '#475569' }}>
                No active subscription. Subscribe to a plan to start using AirDesk.
              </p>
              <Button
                asChild
      ...
      {hasActiveSubscription && (
        <Card className="bg-white rounded-xl shadow-sm" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }}>
          <CardHeader>
            <CardTitle style={{ color: '#1E40AF' }}>Billing History</CardTitle>
            <CardDescription style={{ color: '#475569' }}>
          ...
          <CardContent>
            {loadingInvoices ? (
              <div className="flex items-center justify-center py-8">
                <Loader2 className="h-6 w-6 animate-spin" style={{ color: '#475569' }} />
              </div>
            ) : invoices.length === 0 ? (
              <div className="text-center py-8">
                <FileText className="h-12 w-12 mx-auto mb-4" style={{ color: '#475569', opacity: 0.5 }} />
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: '#475569' }}>
                        Invoice
                      </th>
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: '#475569' }}>
                        Date
                  ...
                  <tbody>
                    {invoices.map((invoice) => (
                      <tr key={invoice.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-4 text-sm" style={{ color: '#1E40AF' }}>
                          {invoice.number || invoice.id.slice(-8)}
                        </td>
                        <td className="px-4 py-4 text-sm" style={{ color: '#475569' }}>
                          {formatDate(invoice.created)}
                        </td>
                        <td className="px-4 py-4 text-sm font-medium" style={{ color: '#1E40AF' }}>
