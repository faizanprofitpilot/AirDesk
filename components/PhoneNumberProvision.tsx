    ...
    return (
      ...
      >
        ...
        {firm?.vapi_phone_number_id && !firm?.inbound_number_e164 && (
          <p className="text-sm mt-2" style={{ color: '#475569', opacity: 0.7 }}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // If server returned both a generic error + details, include details to make debugging possible
        if (data?.error && data?.details && !data?.message) {
          const detailsStr = typeof data.details === 'string' ? data.details : JSON.stringify(data.details);
          errorMsg = `${data.error}: ${detailsStr}`;
        }
          ...
          if (response.status === 409 || errorMsg.includes('already generated') || errorMsg.includes('already provisioned')) {
            errorMsg = 'A phone number has already been generated for this business. Only one number is allowed per business.';
