  ...
  if (shouldSendEmail) {
    ...
    try {
      await sendIntakeEmail(
        ...
        (currentFirm as any).send_incomplete_tickets || false, // Whether to send incomplete tickets
        (currentFirm as any).cc_emails || [], // Optional CC list
        call.id, // Call ID for dashboard link
        (currentFirm as any).service_call_fee // Service call fee
      );
