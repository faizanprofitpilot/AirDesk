...
export interface Firm {
  ...
  cc_emails?: string[]; // Optional CC list for tickets
  agent_name?: string; // AI agent name (e.g., "Jessica")
  business_hours_open?: string; // e.g., "09:00"
  business_hours_close?: string; // e.g., "17:00"
  after_hours_handling?: 'message_only' | 'collect_scheduling'; // After-hours behavior
  default_next_available_slot?: string; // e.g., "Tomorrow morning at 8:00 a.m."
  service_call_fee?: number; // Service call fee amount
  service_fee_enabled?: boolean; // Whether to mention pricing
  send_incomplete_tickets?: boolean; // Send tickets even if missing required fields
  ...
  open_time?: string; // Deprecated - use business_hours_open
  close_time?: string; // Deprecated - use business_hours_close
  ...
  ai_tone?: AITone; // Deprecated - kept for backward compatibility
  ai_custom_prompt?: string | null; // Custom system prompt for AI behavior
...
export interface IntakeData {
  ...
  city?: string;
  state?: string;
  issueCategory?: 'No heat' | 'No cool' | 'Furnace' | 'AC' | 'Thermostat' | 'Strange noise' | 'Leak' | 'Other';
  issueDescription?: string;
  urgency?: 'ASAP' | 'can wait';
  requestedWindow?: string; // Preferred appointment time
  nextAvailableOffered?: boolean; // Whether we offered "next available"
  serviceFeeMentioned?: boolean; // Whether pricing was discussed
  notes?: string;
